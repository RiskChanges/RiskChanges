...
def getEARJSON(con,earid,storedir='/temp/'):
    ...
    try:
        ...
        print(f"couldn't save the file at {filepath} please check filename directory and required permissions")
    return filepath

def getExposureJSON(con,exposureid):
    ...
    try:
        ...
        print(f"couldn't save the file at {filepath} please check filename directory and required permissions")
    return filepath
def getLossJSON(con,lossid):
    loss_geom=readLossGeom(con,exposureid)
    filename=name_generator()
    filepath=f"{storedir}{filename}.geojson"
    try:
        ...
        print(f"couldn't save the file at {filepath} please check filename directory and required permissions")
    return filepath
def getRiskJSON(con,riskid):
    risk_geom=readRiskGeometry(con,exposureid)
    filename=name_generator()
    filepath=f"{storedir}{filename}.geojson"
    try:
